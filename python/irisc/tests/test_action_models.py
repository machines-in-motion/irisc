...
def test_point_cliff_derivatives(): 
    ...
    for i, m in enumerate(models): 
        ...
        np.testing.assert_almost_equal(datas[i].Lx, cost_numdiff[i].Lx , decimal=5, err_msg="Error in Lx for model {}".format(i+1))
        cost_numdiff[i].calcLu(x,u)
        np.testing.assert_almost_equal(datas[i].Lu, cost_numdiff[i].Lu , decimal=5, err_msg="Error in Lu for model  {}".format(i+1))
        cost_numdiff[i].calcLxx(x,u)
        np.testing.assert_almost_equal(datas[i].Lxx, cost_numdiff[i].Lxx , decimal=5, err_msg="Error in Lxx for model {}".format(i+1))
        cost_numdiff[i].calcLuu(x,u)
        np.testing.assert_almost_equal(datas[i].Luu, cost_numdiff[i].Luu , decimal=5, err_msg="Error in Luu for model {}".format(i+1))
        cost_numdiff[i].calcLxu(x,u)
        np.testing.assert_almost_equal(datas[i].Lxu, cost_numdiff[i].Lxu , decimal=5, err_msg="Error in Lxu for model {}".format(i+1))
        # # DYNAMICS TESTING 
        dynamics_numdiff[i].calcFx(x,u) 
        np.testing.assert_almost_equal(datas[i].Fx, dynamics_numdiff[i].Fx , decimal=5, err_msg="Error in Fx for model {}".format(i+1))
        dynamics_numdiff[i].calcFu(x,u) 
        np.testing.assert_almost_equal(datas[i].Fu, dynamics_numdiff[i].Fu , decimal=5, err_msg="Error in Fu for model {}".format(i+1))
